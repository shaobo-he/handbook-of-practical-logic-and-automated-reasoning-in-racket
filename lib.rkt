...
(define (psubset x y)
  (let ([sx (list->set x)] [sy (list->set y)])
    (and (subset? sx sy) (not (set=? sx sy)))))

(define (set-eq x y) (set=? (list->set x) (list->set y)))
...
(define (allpairs f xs ys)
  (for*/list ([x (in-list xs)] [y (in-list ys)]) (f x y)))

;; complement of a predicate
(define (non p) (λ (x) (not (p x))))

;; #t if (f x) doesn't raise
(define (can f x) (with-handlers ([exn:fail? (λ (e) #f)]) (f x) #t))

;; apply f to x, n times
(define (funpow n f x) (if (<= n 0) x (funpow (- n 1) f (f x))))

;; smallest n >= start with (p n) true
(define (first n p) (if (p n) n (first (+ n 1) p)))

;; apply f repeatedly until it raises, returning the last good value
(define (repeat f x) (with-handlers ([exn:fail? (λ (e) x)]) (repeat f (f x))))

;; closure-based finite functions (used for model interpretations)
(define (undef x) (error 'undef "undefined function"))
(define (valmod a y f) (λ (x) (if (equal? x a) y (f x))))

;; add an element to a set (no-op if already present)
(define (insert x s) (if (member x s) s (cons x s)))

;; element of l minimizing / maximizing f
(define (minimize f l) (argmin f l))
(define (maximize f l) (argmax f l))
...
(define (mapfilter f l)
  (filter-map (λ (x) (with-handlers ([exn:fail? (λ (e) #f)]) (f x))) l))

;; all subsets / all non-empty subsets of a (duplicate-free) list
(define (allsubsets s)
  (if (null? s)
      (list '())
      (let ([res (allsubsets (cdr s))])
        (append (map (λ (b) (cons (car s) b)) res) res))))
...
(define (allsets m l)
  (cond
    [(= m 0) (list '())]
    [(null? l) '()]
    [else (union (image (λ (g) (cons (car l) g)) (allsets (- m 1) (cdr l)))
                 (allsets m (cdr l)))]))

;; 0-based position of x in l (raises if absent)
(define (index x l)
  (let loop ([l l] [i 0])
    (cond [(null? l) (error 'index "not found")]
          [(equal? (car l) x) i]
          [else (loop (cdr l) (add1 i))])))

;; all but the last element
(define (butlast l) (if (null? (cdr l)) '() (cons (car l) (butlast (cdr l)))))
...
(define (update k v f) (hash-set f k v))
(define (undefine k f) (hash-remove f k))

(define (apply f k)
  (hash-ref f k (λ () (error 'apply "argument not in domain: ~s" k))))

(define (tryapplyd f k d) (hash-ref f k d))
(define (tryapplyl f k) (tryapplyd f k '()))
(define (defined f k) (hash-has-key? f k))

;; build a partial function from parallel key/value lists
(define (fpf keys vals) (make-immutable-hash (map cons keys vals)))

;; enumeration / transformation
(define (dom f) (hash-keys f))
(define (graph f) (hash->list f))
(define (mapf g f) (for/hash ([(k v) (in-hash f)]) (values k (g v))))
...
(define (tryterminus ptn a)
  (with-handlers ([exn:fail? (λ (e) (cons a 1))]) (terminus ptn a)))

(define (canonize ptn a) (car (tryterminus ptn a)))

(define (equivalent eqv a b) (equal? (canonize eqv a) (canonize eqv b)))
...
(define (equate ab ptn)
  ...
  (define a* (car ta)) (define na (cdr ta))
  (define b* (car tb)) (define nb (cdr tb))
  (cond
    [(equal? a* b*) ptn]
    [(<= na nb)
     (update a* (cons 'nonterminal b*)
             (update b* (list 'terminal b* (+ na nb)) ptn))]
    [else
     (update b* (cons 'nonterminal a*)
             (update a* (list 'terminal a* (+ na nb)) ptn))]))

(define (equated ptn) (dom ptn))
