...
(require (only-in "lib.rkt" fpf update undefined union subtract set-eq))
(require (only-in "formulas.rkt" onatoms conjuncts dest-imp end-itlist))
(require (only-in "fol.rkt" subst fv fvt tsubst))
(require (only-in "cooper.rkt" zero))
(require (only-in "complex.rkt" is-constant degree pdivide coefficients head polyatom))
(require (only-in "equal.rkt" mk-eq lhs))
