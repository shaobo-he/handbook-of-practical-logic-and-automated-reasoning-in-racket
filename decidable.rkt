...
(require (only-in "formulas.rkt" list-conj list-disj mk-and mk-imp))
(require (only-in "prop.rkt" simpcnf simpdnf purednf))
(require (only-in "fol.rkt" fv functions generalize holds subst))
(require (only-in "skolem.rkt" skolemize askolemize simplify nnf specialize pnf))
(require (only-in "herbrand.rkt" groundtuples))
(require (only-in "dp.rkt" dpll))
(require (only-in "equal.rkt" predicates))
(require (only-in "meson.rkt" contrapositives mexpand002))
