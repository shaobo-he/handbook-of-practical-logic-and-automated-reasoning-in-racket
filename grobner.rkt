...
(require (only-in "lib.rkt" funpow union tryfind distinctpairs))
(require (only-in "formulas.rkt" end-itlist))
(require (only-in "prop.rkt" positive negate simpdnf))
(require (only-in "fol.rkt" fv variant))
(require (only-in "skolem.rkt" specialize prenex nnf simplify))
(require (only-in "cooper.rkt" dest-numeral mk-numeral))
(require (only-in "order.rkt" lexord))
...
;; ===== monomial operations =====
(define (mmul cm1 cm2) (cons (* (car cm1) (car cm2)) (map + (cdr cm1) (cdr cm2))))

(define (mdiv cm1 cm2)
  (define (index-sub n1 n2) (if (< n1 n2) (error 'mdiv "mdiv") (- n1 n2)))
  (cons (/ (car cm1) (car cm2)) (map index-sub (cdr cm1) (cdr cm2))))

(define (mlcm cm1 cm2) (cons 1 (map max (cdr cm1) (cdr cm2))))
...
(define (mpoly-mmul cm pol) (map (λ (m) (mmul cm m)) pol))
(define (mpoly-neg pol) (map (λ (cm) (cons (- (car cm)) (cdr cm))) pol))
(define (mpoly-const vars c) (if (= c 0) '() (list (cons c (map (λ (_) 0) vars)))))
(define (mpoly-var vars x) (list (cons 1 (map (λ (y) (if (equal? y x) 1 0)) vars))))
...
(define (mpoly-pow vars l n) (funpow n (λ (z) (mpoly-mul l z)) (mpoly-const vars 1)))

(define (mpoly-inv p)
  (match p
    [(list (cons c m)) #:when (andmap (λ (i) (= i 0)) m) (list (cons (/ 1 c) m))]
    [_ (error 'mpoly-inv "non-constant polynomial")]))

(define (mpoly-div p q) (mpoly-mul p (mpoly-inv q)))
...
(define (reduceb cm pols) (tryfind (λ (p) (reduce1 cm p)) pols))
...
(define (grobner basis pairs)
  (when (grobner-verbose) (printf "~a basis elements and ~a pairs\n" (length basis) (length pairs)))
  (match pairs
    ['() basis]
    [(cons p1p2 opairs)
...
(define (groebner basis) (grobner basis (distinctpairs basis)))
...
(define (grobner-trivial fms)
  ...
  (define rvs (map (λ (n) (variant (string->symbol (string-append "_" (number->string n))) vars0))
...
(define (term-of-varpow vars xk)
  (if (= (cdr xk) 1) `(var ,(car xk)) `(fn ^ (var ,(car xk)) ,(mk-numeral (cdr xk)))))
