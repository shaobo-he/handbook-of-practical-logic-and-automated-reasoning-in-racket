...
(define (tsubst sfn tm)
  (match tm
    [`(var ,vn) (with-handlers ([exn:misc:match? (λ (e) tm)]) (sfn vn))]
    [`(fn ,f ,@args) `(fn ,f ,@(map (λ (t) (tsubst sfn t)) args))]))

(define (variant x vars)
  (if (member x vars) (variant (string->symbol (string-append (symbol->string x) "^")) vars) x))
...
(define (substq subfn q x p)
  (define x^
    (if (ormap (λ (y) (member x (fvt (tryapplyd subfn y `(var ,y))))) (subtract (fv p) `(,x)))
