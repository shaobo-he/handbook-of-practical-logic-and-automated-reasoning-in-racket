...
(define (lexwhile prop inp)
  (match inp
    [(cons c cs)
     #:when (prop c)
     (let-values ([(tok rest) (lexwhile prop cs)]) (values (cons c tok) rest))]
    [_ (values '() inp)]))
...
(define (lex inp)
  (let-values ([(_ rest) (lexwhile space inp)])
    (match rest
      ...
      [(cons c cs)
       (define prop
         (cond [(alphanumeric c) alphanumeric]
               [(symbolic c) symbolic]
               [else (λ (c) #f)]))
...
(define (parse-expression i)
  (let-values ([(e1 i1) (parse-product i)])
    (match i1
      [(cons "+" i1b) (let-values ([(e2 i2) (parse-expression i1b)]) (values `(add ,e1 ,e2) i2))]
      [_ (values e1 i1)])))

(define (parse-product i)
  (let-values ([(e1 i1) (parse-atom i)])
    (match i1
      [(cons "*" i1b) (let-values ([(e2 i2) (parse-product i1b)]) (values `(mul ,e1 ,e2) i2))]
      [_ (values e1 i1)])))
...
(define (make-parser pfn s)
  (let-values ([(expr rest) (pfn (lex (string->list s)))])
    (if (null? rest) expr (error 'parse "Unparsed input"))))

(define (parse-exp s) (make-parser parse-expression s))
...
(define (string-of-exp pr e)
  (match e
    ...
    [`(add ,e1 ,e2)
     ...
     (if (< 2 pr) (string-append "(" s ")") s)]
    [`(mul ,e1 ,e2)
     (define s (string-append (string-of-exp 5 e1) " * " (string-of-exp 4 e2)))
     (if (< 4 pr) (string-append "(" s ")") s)]))

(define (sprint-exp e) (string-append "<<" (string-of-exp 0 e) ">>"))
