...
(require (only-in "lib.rkt" index butlast insertat chop-list subtract))
(require (only-in "formulas.rkt" atom-union))
(require (only-in "prop.rkt" eval dnf))
...
(require (only-in "complex.rkt"
                  poly-cmul poly-neg poly-mul head degree is-constant behead
                  pdivide findsign assertsign split-zero swap polyatom init-sgns))
...
(define (poly-diffn x n p)
  (match p
    [`(fn + ,c (fn * ,y ,q)) #:when (equal? y x)
     `(fn + ,(poly-cmul n c) (fn * ,x ,(poly-diffn x (+ n 1) q)))]
    [_ (poly-cmul n p)]))

(define (poly-diff vars p)
  (match p
    [`(fn + ,c (fn * (var ,x) ,q)) #:when (equal? x (car vars))
...
(define (testform pmat fm)
  (eval fm (λ (at)
             (match at
               [`(rel ,a ,p ,z)
                (and (member (cdr (assoc p pmat)) (cdr (assoc a rel-signs))) #t)]))))

;; ===== sign inference =====
(define (inferpsign pdqd)
  (define pd (car pdqd)) (define qd (cdr pdqd))
...
(define (condense ps)
  (match ps
    [(list* iv pt other)
     ...
     (if (member 'zero pt) (cons iv (cons pt rest)) rest)]
    [_ ps]))
  ...
  (match ps
    [(list* (and x (cons l ls)) (cons _ ints) (and pts (cons (cons r rs) xs)))
     (match* (l r)
       ...
       [(_ _) (cons x (cons (cons l ints)
                            (cons (cons 'zero ints) (cons (cons r ints) (inferisign pts)))))])]
    [_ ps]))

(define (dedmatrix cont mat)
  (define l (quotient (length (car mat)) 2))
  (define mat1 (condense (map (λ (row) (let-values ([(a b) (chop-list l row)]) (inferpsign (cons a b)))) mat)))
  (define mat2 (cons (list (swap #t (list-ref (car mat1) 1)))
...
(define (casesplit vars dun pols cont sgns)
  (match pols
    ...
    [(cons p ops)
     (define zbranch
       (if (is-constant vars p) (λ (s) (delconst vars dun p ops cont s))
...
(define (delconst vars dun p ops cont sgns)
  (define (cont* m) (cont (map (λ (row) (insertat (length dun) (findsign sgns p) row)) m)))
  (casesplit vars dun ops cont* sgns))

(define (matrix vars pols cont sgns)
  (if (null? pols)
      (with-handlers ([exn:fail? (λ (e) #f)]) (cont (list '())))
      (let ()
        ...
        (define (cont* m) (cont (map (λ (l) (insertat i (car l) (cdr l))) m)))
...
(define (basic-real-qelim vars fm)
  (match fm
    [`(exists ,x ,p)
     (define pols (atom-union (λ (at) (match at [`(rel ,a ,t (fn |0|)) (list t)] [_ '()])) p))
     (define (cont mat) (if (ormap (λ (m) (testform (map cons pols m) p)) mat) #t #f))
     (casesplit (cons x vars) '() pols cont init-sgns)]))

(define real-qelim
  (λ (fm)
    (simplify (evalc ((lift-qelim polyatom (λ (g) (simplify (evalc g)))
                                  (λ (vars) (λ (f) (basic-real-qelim vars f)))) fm)))))

(define real-qelim*
  (λ (fm)
    (simplify (evalc ((lift-qelim polyatom (λ (g) (dnf ((cnnf (λ (x) x)) (evalc g))))
                                  (λ (vars) (λ (f) (basic-real-qelim vars f)))) fm)))))
