...
(require (only-in "lib.rkt" update undefined allpairs subtract))
(require (only-in "formulas.rkt" conjuncts disjuncts list-conj list-disj mk-and))
(require (only-in "prop.rkt" dnf negate))
(require (only-in "fol.rkt" fv subst))
(require (only-in "skolem.rkt" simplify))
(require (only-in "decidable.rkt" miniscope))
(require (only-in "equal.rkt" is-eq dest-eq))
